...
import {
  Plus, Pencil, Trash2, X, BookOpen, Copy,
...
export default function AdminGuidesPage() {
  ...
  const loadGuides = useCallback(async () => {
    ...
    setGuides(
      (guidesData ?? []).map((g) => ({ ...g, section_count: sectionCounts[g.id] ?? 0 }))
    )
    setLoading(false)
  }, [supabase])
  ...
  async function openDuplicate(guide: Guide) {
    ...
    setForm({
      title: guide.title,
      slug: (guide.slug ?? slugify(guide.unit_code + " " + guide.title)) + "-copy",
      unit_code: guide.unit_code,
      unit_name: guide.unit_name,
      author: guide.author ?? "",
      cover_image_url: guide.cover_image_url ?? "",
    ...
    const { data } = await supabase
      ...
      .order("display_order")
    setSections(
      (data ?? []).map((s) => ({ ...s, id: makeTempId(), _tempId: makeTempId() }))
    )
    setSheetOpen(true)
  }

  function updateUnitOrTitle(field: "unit_code" | "title", value: string) {
    const nextForm = { ...form, [field]: field === "unit_code" ? value.toUpperCase() : value }
  ...
  function addSection() {
    setSections((prev) => [
      ...prev,
      { id: makeTempId(), _tempId: makeTempId(), guide_id: "", question: "", answer: "", display_order: prev.length },
    ])
  }

  function updateSection(tempId: string, field: "question" | "answer", value: string) {
    setSections((prev) =>
  ...
  function deleteSection(tempId: string) {
    setSections((prev) =>
      prev.filter((s) => s._tempId !== tempId).map((s, i) => ({ ...s, display_order: i }))
    )
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (editing) {
      ...
      if (error) { addToast("Failed to update guide.", "error"); setSaving(false); return }
      guideId = editing.id
    } else {
      const { data, error } = await supabase.from("guides").insert([payload]).select("id").single()
      if (error || !data) { addToast("Failed to create guide.", "error"); setSaving(false); return }
      guideId = data.id
    }
    ...
    if (sections.length) {
      ...
      const { error } = await supabase.from("guide_sections").insert(sectionPayloads)
      if (error) { addToast("Guide saved but sections failed to save.", "error") }
    }
  ...
  async function handleTogglePublished(guide: Guide) {
    ...
    setGuides((prev) =>
      prev.map((g) => g.id === guide.id ? { ...g, published: !g.published } : g)
    )
  }
  ...
  async function handleDelete() {
    ...
    const { error } = await supabase.from("guides").delete().eq("id", deleteTarget.id)
    if (error) { addToast("Failed to delete guide.", "error") }
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted mt-1 text-sm">Add, edit, and organise unit survival guides.</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          ...
          <div key={s.label} className="bg-white rounded-xl border border-border p-4 shadow-sm text-center">
            <p className={cn("font-display text-3xl", s.color)}>{s.value}</p>
            <p className="text-xs text-muted mt-0.5 font-medium uppercase tracking-wider">{s.label}</p>
          </div>
        ))}
      </div>
        ...
        {loading ? (
          ...
          <div className="p-14 text-center">
            ...
            <p className="text-sm text-muted">
              {search ? "Try a different search term." : "Add your first guide to get started."}
            </p>
          </div>
        ) : (
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="border-b border-border bg-cream">
                <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-muted">Unit</th>
                <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-muted hidden md:table-cell">Author</th>
                <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-muted">Difficulty</th>
                <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-muted hidden sm:table-cell">Year</th>
                <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-muted hidden lg:table-cell">Sections</th>
                <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-muted">Published</th>
              ...
              {filtered.map((guide) => (
                ...
                >
                  <td className="px-5 py-4">
                    <p className="text-xs font-bold font-mono text-primary">{guide.unit_code}</p>
                  ...
                  <td className="px-5 py-4">
                    {guide.difficulty ? (
                      <span className={cn("text-xs rounded-full px-2.5 py-0.5 font-medium capitalize", diffBadge(guide.difficulty))}>
                        {guide.difficulty}
                      </span>
                    ) : "—"}
                  </td>
                  <td className="px-5 py-4 hidden sm:table-cell">
                    <p className="text-sm text-muted">{guide.year_level ?? "—"}</p>
                  </td>
                  <td className="px-5 py-4 hidden lg:table-cell">
                    <p className="text-sm text-muted">{guide.section_count ?? 0} sections</p>
                  </td>
                  <td className="px-5 py-4">
                    <button
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField label="Unit Code" required>
                <input
                  ...
                  onChange={(e) => updateUnitOrTitle("unit_code", e.target.value)}
            ...
            {form.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {form.tags.map((tag) => (
                  <span key={tag} className="text-[11px] font-medium bg-primary/10 text-primary px-2.5 py-0.5 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="relative flex items-center gap-4 py-2">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-muted shrink-0">Content</span>
              <div className="flex-1 h-px bg-border" />
            </div>

            {/* ── Part B: Content ── */}
            <FormField label="Opening Introduction" hint="Reflective opening paragraph">
              <textarea
                rows={4}
                value={form.intro}
            ...
            <div>
              <div className="flex items-baseline justify-between mb-2">
                ...
                <span className="text-[10px] text-muted">{sections.length} section{sections.length !== 1 ? "s" : ""}</span>
              </div>
                ...
                {sections.map((section, i) => (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <GripVertical size={14} className="text-muted mt-2.5 shrink-0" />
                      <div className="flex-1 space-y-2">
                        <input
                          type="text"
                          value={section.question}
                          onChange={(e) => updateSection(section._tempId, "question", e.target.value)}
                        ...
                        <textarea
                          ...
                          onChange={(e) => updateSection(section._tempId, "answer", e.target.value)}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-border shadow-2xl p-8 max-w-sm w-full">
            ...
            <h3 className="font-display text-xl text-text text-center mb-1">Delete Guide?</h3>
            <p className="text-sm text-muted text-center mb-6">
              Are you sure you want to delete{" "}
              <span className="font-semibold text-text">&ldquo;{deleteTarget.title}&rdquo;</span>?
              All Q&amp;A sections will also be deleted. This cannot be undone.
            </p>
