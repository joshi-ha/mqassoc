...
export function GuideCard({ guide }: { guide: Guide }) {
  ...
  return (
    <Link
      href={href}
      className="group flex flex-col bg-white rounded-2xl border border-border p-6 hover:shadow-lg hover:border-primary/40 transition-all duration-300"
    >
      {/* Top row: unit code pill + difficulty badge */}
      <div className="flex items-center justify-between gap-3 mb-3">
        <span className="inline-block text-xs font-bold font-mono tracking-wider px-2.5 py-1 rounded-full bg-[color-mix(in_srgb,var(--color-primary)_10%,transparent)] text-primary">
          {guide.unit_code}
        </span>
        {diff && (
          <span
      ...
      {/* Unit name */}
      <p className="text-xs text-muted mb-1">{guide.unit_name}</p>

      {/* Title */}
      <h3 className="font-display text-lg text-text leading-snug mb-2 group-hover:text-primary transition-colors">
        {guide.title}
      </h3>

      {/* Author */}
      {guide.author && (
        <p className="text-xs italic text-muted mb-3">By {guide.author}</p>
      )}
